...
const GetBookmarkedItems = () => {
    ...
    $J(`div.item.app${appid}[id*=${appid}_]`).each((idx, elem) => {
        const $elem = $J(elem);
        const market_hash_name = elem.rgItem.description ? elem.rgItem.description.market_hash_name : elem.rgItem.market_hash_name;
    ...
    if (appid == 440 || appid == 570) {
        chrome.runtime.sendMessage(SIHID, { type: 'GetPlayerItems', steamid, appid }, function (response) {
            ...
            if (response.success && response.data) {
                apiItems[steamid][appid] = response.data;
                $J.each(apiItems[steamid][appid], function (i, o) {
                    var elIt = $J(`div.item[id*=${appid}_2_${o.id}]`);
                    if (o.equipped) {
